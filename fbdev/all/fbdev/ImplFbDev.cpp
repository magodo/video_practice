...
static std::string Fourcc2String(uint32_t pix_fmt)
{
    std::string str;
...
static uint32_t bppOfFmt(uint32_t pix_fmt)
{
    switch (pix_fmt)
    {
        ...
        case V4L2_PIX_FMT_RGB565:
            return 16;
        default:
            std::cerr << "Unsupported format: " << Fourcc2String(pix_fmt) << std::endl;
            return 0;
    }
}
...
bool ImplFbDev::init(string dev)
{
    ...
    m_vinfo.nonstd = StrToFourcc(parser->getInfo()["FB"]["pix_fmt"]);
    m_vinfo.bits_per_pixel = bppOfFmt(m_vinfo.nonstd);
